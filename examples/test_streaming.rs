...
use anthropic_sdk::{
    Anthropic,
    types::{MessageCreateBuilder, ContentBlock}
};
    ...
    while let Some(result) = stream.next().await {
        ...
        match result {
            Ok(event) => {
                ...
                println!("✅ Processing event type: {:?}", std::mem::discriminant(&event));
                io::stdout().flush().unwrap();

                match event {
                    MessageStreamEvent::ContentBlockStart { content_block, index } => {
                        println!("🎬 ContentBlockStart - Index: {}, Type: {:?}", index, std::mem::discriminant(&content_block));
                        match content_block {
                    ...
                    MessageStreamEvent::ContentBlockDelta { delta, index } => {
                        ...
                        println!("📝 ContentBlockDelta - Index: {}, Delta type: {:?}", index, std::mem::discriminant(&delta));
                        match delta {
