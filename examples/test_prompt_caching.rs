...
async fn conversation_context_caching(client: &Anthropic) -> Result<(), Box<dyn Error>> {
    ...
    let conversation_history = vec![
        ("user", "I'm building a new SaaS application for project management. Can you help me design the architecture?"),
    ...
    for (_i, (role, content)) in conversation_history.iter().enumerate() {
        let role = match *role {
            "user" => Role::User,
